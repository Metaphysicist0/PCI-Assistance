...
class Trainer:
    ...
    def __init__(self, path, model, model_copy, img_save_path):
        ...
        # self.net = ResUNet.ResUnet(3).to(self.device)
        # self.net = ResUnetPPP.ResUNetPP(nums_layer).to(self.device)
        # self.net = ResNeXt.resnext50().to(self.device)
        # self.net = ResUnetPPP.RetinexNetResUNetPP(nums_layer).to(self.device)
        # self.net = ResUNet.ResUnet(3).to(self.device)
        # self.net = TransResUNet.TransResUNet().to(self.device)
        # self.net = mobilenetv2.mobilenetv2().to(self.device)
...
if __name__ == '__main__':
    # 路径改一下
    t = Trainer(r"bigDRIVE/training", r'./model.pth', r'./model_{}_{}.pth', img_save_path=r'./train_img')
    t.train(300)

    # 25 epoch 20230509 21:17
    # to 30 epoch 20230510 10:23
    # UNet
    # to 25 epoch 20230511 17:20