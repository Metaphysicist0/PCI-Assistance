...
class Datasets(Dataset):
    ...
    def __init__(self, path):
        self.path = path
        # 语义分割需要的图片的图片和标签
        self.name1 = sorted(os.listdir(os.path.join(path, "images")))
        self.name2 = sorted(os.listdir(os.path.join(path, "1st_manual")))
    ...
    def __getitem__(self, index):
        ...
        res, img_l = cv2.threshold(img_l, 12, 255,cv2.THRESH_BINARY)
...
if __name__ == '__main__':
    i = 1
    # dataset = Datasets(r"E:\ResUNetPP\bigDRIVE\training")
    dataset = Datasets('./bigDRIVE/training')
    for a, b in dataset:
        print(i)
        print(a.shape)
        print(b.shape)
        save_image(a, f"./{i}.jpg", nrow=1)
        save_image(b, f"./{i}_mask.jpg", nrow=1)
        break
        i += 1
        if i > 5:
            break
